...
define( 'CUSTOM_METABOXES_DIR', get_template_directory_uri() . '/includes/metaboxes' );
...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		// checkbox
		case 'checkbox':
			echo '<div class="slideThree"><input type="checkbox" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" ' . checked( $meta, true, false ) . ' value="1" />
					<label for="' . esc_attr( $id ) . '"></label></div>' . $desc ;
		break;
		// select, chosen
		case 'select':
		case 'chosen':   
...
class Custom_Add_Meta_Box {
	...
	function admin_enqueue_scripts() {
		...
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
           ...
           wp_enqueue_script( 'meta_box_ui', CUSTOM_METABOXES_DIR . '/js/jquery-ui-1.9.2.custom.min.js', $deps );
				wp_enqueue_script( 'meta_box', CUSTOM_METABOXES_DIR . '/js/scripts.js', $deps,"1.0.1",true);
			
			// css
			$deps = array();
			wp_register_style( 'jqueryui', CUSTOM_METABOXES_DIR . '/css/jquery-ui-1.9.2.custom.min.css' );
			if ( meta_box_find_field_type( 'date', $this->fields ) || meta_box_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jqueryui';
			if ( meta_box_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
                wp_enqueue_style( 'jqueryui', CUSTOM_METABOXES_DIR . '/css/jquery-ui-1.9.2.custom.min.css', $deps );
                wp_enqueue_style( 'meta_box', CUSTOM_METABOXES_DIR . '/css/meta_box.css', $deps );
			    wp_enqueue_style( 'meta_box_checkboxes', CUSTOM_METABOXES_DIR . '/css/checkbox.css', $deps );
		}
	}
